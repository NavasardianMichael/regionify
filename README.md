...
4. Configure Nginx as **two separate server config files**, one per host: `deployment/regionify.pro.conf` (SPA host — serves static files from `$APP_DIR/client/current`, proxies the collision-free public pages plus `/embed/<token>` to Node, and falls back to the static SPA shell for everything else) and `deployment/api.regionify.pro.conf` (API host — proxies everything to **`127.0.0.1:9002`** preserving the URI). See `docs/SEO_INDEXING_ARCHITECTURE.md` for why only some SPA-host routes are proxied to Node. For marketing routes, keep `try_files $uri $uri/ =404;` together with `error_page 404 /marketing/404.html;` so unsupported countries and unknown `/marketing/*` paths render Astro’s branded 404 page with a real 404 status.
5. **Public embed SSR:** `GET /embed/<token>` is rendered by Express on the SPA host with the project’s SEO title, description, keywords, `<h1>`, and JSON-LD. `deployment/regionify.pro.conf` proxies `^/embed/[^/]+$` to Node **before** the `location /` `try_files` block so embed URLs are not served as the generic SPA `index.html`.
