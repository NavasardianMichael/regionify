...
async function generateFavicons() {
  ...
  // Trim whitespace from the logo first for better icon fill
  const trimmedLogo = sharp(LOGO_ICON).trim();
  ...
  const ico16 = await trimmedLogo
    .clone()
    .resize(16, 16, { fit: 'contain', background: { r: 255, g: 255, b: 255, alpha: 0 } })
    .png()
    .toBuffer();

  const ico32 = await trimmedLogo
    .clone()
    .resize(32, 32, { fit: 'contain', background: { r: 255, g: 255, b: 255, alpha: 0 } })
    .png()
    .toBuffer();

  const ico48 = await trimmedLogo
    .clone()
