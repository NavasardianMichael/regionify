...
• Chronographer ($149/mo) — time-series data, animated exports, public embeds, AI Agent (parse & generate)
...
Regenerate any time by running the script. It captures a ~90-110 s workflow: land on the new-project page → pick France → sample data loads → switch to AI Agent import → type a plain-English prompt ("average annual temperature for each region, 2015-2024") → AI streams structured data live → auto-switches to table view → Save, map + timeline populate → expand Legend Configuration → Ranges → Normalize ranges → hit Play and watch the map animate through all 10 years → Export → format Video (MP4) → Download, MP4 renders on-camera. Landscape 1280 × 960, no audio.
...
- Recommended polish before posting (do this in Descript, CapCut, or Screen Studio):
  1. Add burned-in captions on the two "wow" moments — the AI prompt streaming into real structured data ("Type a sentence → get a full dataset") and the timeline playing through all 10 years ("Time-series, animated, one click")
  2. Add a title card at 0-2 s: "Regionify — from a sentence to an animated regional map"
  3. Add an end card in the final 2-3 s: "Try free at regionify.pro"
  4. Optional: crop to 1:1 square (1080 × 1080) — LinkedIn's algorithm slightly favours it over landscape
  5. Consider trimming the AI-generation wait (~15-45 s of streaming) with a speed-ramp or jump-cut — it's the least visually active stretch of the video
