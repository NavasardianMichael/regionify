...
```
Turning a regional CSV into a publishable map used to take me an hour.
...
```
If you've ever pasted a spreadsheet into a mapping tool and watched it fail on "Île-de-France" or "Catalunya" — this one's for you.
...
```
D3.js is the wrong tool for 90% of regional maps people actually need.
...
```
Excel spreadsheet → publishable regional map. Under a minute. No code.
...
```
Regionify is a browser tool that turns region-level data (populations, sales, election results — anything with a region column) into styled, publishable maps.
...
→ Export to PNG, JPEG, PDF, or SVG up to 4K. GIF and MP4 for animated time-series data

→ Publish as an embeddable public page with SEO metadata (paid tier)

→ Localised in 7 languages: EN, DE, FR, ES, PT, RU, ZH

Pricing is freemium — the free plan is genuinely free, no credit card:
• Observer (free) — 5 projects, PNG/JPEG/PDF export, all 200+ maps included
• Explorer ($49/mo) — unlimited projects, SVG + 4K, advanced styles
• Chronographer ($149/mo) — time-series data, animated exports, public embeds, AI parser

Built for analysts, journalists, policy researchers, agency PMs, and anyone who ships regional data visuals more than twice a year.

Link in the first comment. What's the messiest regional dataset you've ever had to clean up before mapping it?

#SaaS #DataVisualization #IndieHacker #BuildInPublic #DataViz
```
...
**File:** `docs/marketing/assets/demo-video.webm` — automatically generated by `pnpm generate-demo-video`.

Regenerate any time by running the script. It captures a ~30 s workflow: land on the new-project page → pick Spain → sample data loads → cycle through two random palettes → open Export modal → hold on the final styled map. Landscape 1280 × 720, no audio.

**Before uploading to LinkedIn:**

- LinkedIn does not accept `.webm` uploads. Convert to `.mp4` first with one of:
  - Local ffmpeg: `ffmpeg -i demo-video.webm -c:v libx264 -pix_fmt yuv420p -crf 22 demo-video.mp4`
  - Online: [cloudconvert.com/webm-to-mp4](https://cloudconvert.com/webm-to-mp4)
  - Upload to YouTube (unlisted), download the transcoded MP4
- Recommended polish before posting (do this in Descript, CapCut, or Screen Studio):
  1. Add burned-in captions on the two "wow" moments — palette swap ("One click → new palette") and Export ("PNG, 4K, ready to publish")
  2. Add a title card at 0-2 s: "Regionify — from Excel to publishable regional map"
  3. Add an end card at 27-30 s: "Try free at regionify.pro"
  4. Optional: crop to 1:1 square (1080 × 1080) — LinkedIn's algorithm slightly favours it over landscape

If you'd rather record a more polished take yourself, use the auto-generated video as a rehearsal reference for the storyboard and Screen Studio's smart-cursor tracking will give you a nicer result.
...
- Format: PNG
- SIMPLEST BUILD PATH:
    1. Run `pnpm generate-marketing-screenshots` — produces `docs/marketing/assets/data-import-panel.png` (with messy CSV visible in the modal)
...
- Slide 2 (features): 4 icons + one-liners — Import • AI-match • Style • Export. Use `docs/marketing/assets/product-overview.png` as a subtle background
...
> Fair point — the ladder is tuned for teams and agencies where $49/mo is negligible. If you're a solo analyst with monthly needs, tell me what you're missing on the free tier and I'll factor it into a possible solo pricing tier.
