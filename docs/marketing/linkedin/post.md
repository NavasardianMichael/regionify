...
**File:** `docs/marketing/assets/video/demo-video.webm` — automatically generated by `pnpm generate-demo-video`.
...
- LinkedIn does not accept `.webm` uploads. Convert to `.mp4` first with one of:
  - Local ffmpeg: `ffmpeg -i video/demo-video.webm -c:v libx264 -pix_fmt yuv420p -crf 22 video/demo-video.mp4` (run from `docs/marketing/assets/`)
