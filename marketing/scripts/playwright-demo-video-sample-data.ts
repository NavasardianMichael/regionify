/**
 * Marketing demo video recorder — sample-data variant of playwright-demo-video.ts.
 *
 * Records a short workflow video showing the zero-effort path: pick a country and the
 * built-in sample dataset applies automatically (no AI generation step). Same recording
 * approach as the AI variant — raw viewport capture, no zooms, no captions, no music.
 * If a marketing-polished result is needed, treat this as a rehearsal take and re-record
 * in a dedicated tool (Screen Studio, Descript, ScreenFlow).
 *
 * Storyboard (~35-45 s) — tutorial: pick a country, sample data applies instantly, export as MP4.
 *   0-3s    land on /projects/new
 *   3-8s    pick "Italy" in the country picker
 *   8-13s   sample data applies automatically (5-year dynamic timeline, no AI step)
 *   13-??s  expand the "Ranges" accordion in Legend Configuration → click "Normalize ranges"
 ...
 *
 * Post-processed via ffmpeg: 2× for the whole video; only the MP4 render wait runs at
 * 4× (there's no AI streaming step in this variant — sample data applies instantly).
 * Without ffmpeg on PATH the raw 1× recording is kept.
 *
 * Output: `docs/marketing/assets/video/sample-data-to-mp4/demo-video.webm`
 *
 * Run:
 *   pnpm --filter @regionify/marketing generate-demo-video-sample-data
 *   pnpm --filter @regionify/marketing generate-demo-video-sample-data -- --headed   # to watch
 *
 * Requires:
 *   marketing/.env with CLIENT_URL, REGIONIFY_EMAIL, REGIONIFY_PASSWORD set.
 *   Account must be on any paid tier (gates the dynamic/timeline sample data — see
 *   `limits.historicalDataImport` in ImportDataPanel.tsx — otherwise sample data is a
 *   single static snapshot and the timeline/Play button never appear).
...
const VIDEO_TMP_DIR = join(ASSETS_ROOT, '.video-tmp-sample-data');
...
/** Extra speed for the MP4 render wait segment (4× vs real-time). */
...
async function recordStoryboard(page: Page): Promise<void> {
  ...
  // 8-13s — sample data applies automatically. Paid-tier accounts land straight in the
  // dynamic/timeline mode with a 5-year sample series (ImportDataPanel.tsx), so — unlike
  // the AI variant — there's no import-mode switch, modal, or generation wait here at all.
  // The "Switch to X data" button is just the readiness signal that the map has painted.
  await page.waitForLoadState('networkidle', { timeout: 20_000 });
  await page
    .getByRole('button', { name: /Switch to (dynamic|static) data/ })
    .waitFor({ timeout: 30_000 });

  // Hold on the freshly-populated animated map so the viewer registers the sample data.
  await page
    .locator('.ant-slider')
  ...
  // ── Legend Configuration → normalize ranges to the sample data ──
  ...
  if (playBtnAppeared) {
    // Read the period count off the slider's own aria attributes so the wait scales with
    // however many periods the sample data actually has (5, per SAMPLE_TIMELINE_YEAR_COUNT
    // in ImportDataPanel.tsx) instead of hardcoding it.
    ...
    const periodCount = maxAttr !== null ? Number(maxAttr) + 1 : 5;
